"""
Dirty Data Cleaning Studio
===========================
An interactive Streamlit front-end for a movie-catalogue dirty-dataset
cleaning pipeline (Title / Genre / Year / Duration / Rating / IMDb / Votes /
Budget / Revenue / Date / Country / Language cleaning + de-duplication).
...
# ----------------------------------------------------------------------
st.set_page_config(
    page_title="Dirty Data Cleaning Studio",
    page_icon="🎬",
    layout="wide",
...
.stApp{
    background:
        radial-gradient(ellipse 90% 60% at 15% -10%, rgba(229,9,20,.10) 0%, transparent 55%),
        radial-gradient(ellipse 70% 50% at 100% 0%, rgba(242,183,5,.06) 0%, transparent 50%),
        radial-gradient(circle at 10% 0%, #17131a 0%, #0B0B0D 45%) fixed;
    color: var(--ink);
}
...
@keyframes glow-pulse{
    0%, 100%{ text-shadow: 0 0 22px rgba(229,9,20,.35), 0 0 55px rgba(229,9,20,.12); }
    50%{ text-shadow: 0 0 34px rgba(229,9,20,.55), 0 0 80px rgba(229,9,20,.22); }
}
@keyframes fade-up{
...
.filmstrip{
    ...
    box-shadow: 0 0 18px rgba(229,9,20,.08);
}
...
/* ---------- HERO ---------- */
.hero-wrap{ padding: 6px 4px 0 4px; animation: fade-up .7s ease; }
.hero-badge{
...
.card::after{
    content:"";
    position:absolute; top:-40%; right:-30%; width:70%; height:180%;
    background: radial-gradient(circle, rgba(229,9,20,.10) 0%, transparent 70%);
    pointer-events:none;
}
.card:hover{
    transform: translateY(-3px);
    box-shadow: 0 10px 30px rgba(229,9,20,.18), 0 0 0 1px rgba(229,9,20,.25);
...
.card .big{ font-family:'Bebas Neue', sans-serif; font-size:2.3rem; color:var(--ink); line-height:1; text-shadow: 0 0 18px rgba(229,9,20,.18); }
...
.pill.raw{ background: rgba(140,6,13,.22); color:#ff8f94; border:1px solid rgba(229,9,20,.4);}
.pill.clean{ background: rgba(47,191,113,.14); color:var(--good); border:1px solid rgba(47,191,113,.4);}
...
.section-title{
    ...
    padding-left: 10px;
    text-shadow: 0 0 20px rgba(229,9,20,.15);
}
...
.subhead{
    ...
    text-transform:uppercase; color: var(--ink-dim); margin: 4px 0 10px 0;
    display:flex; align-items:center; gap:8px;
}
.subhead::before{ content:""; width:8px; height:8px; border-radius:2px; background: var(--red); box-shadow:0 0 8px var(--red); }
.subhead.clean::before{ background: var(--good); box-shadow:0 0 8px var(--good); }
...
.stButton>button:hover, .stDownloadButton>button:hover{
    transform: translateY(-1px); box-shadow: 0 6px 26px rgba(229,9,20,.5);
}
...
.stTabs [data-baseweb="tab"]{
    ...
    padding: 10px 16px; font-weight:600; transition: color .15s ease;
}
.stTabs [data-baseweb="tab"]:hover{ color: var(--ink); }
.stTabs [aria-selected="true"]{
    color: var(--ink) !important;
    border-bottom: 3px solid var(--red) !important;
    text-shadow: 0 0 14px rgba(229,9,20,.35);
}
...
.brand{
    ...
    text-shadow: 0 0 18px rgba(229,9,20,.25);
}
...
# ----------------------------------------------------------------------
with st.sidebar:
    st.markdown('<div class="brand">DIRTY<span>CLEAN</span></div>', unsafe_allow_html=True)
    st.caption("Dirty Data Cleaning Studio · v1.0")
    st.markdown("---")
...
st.markdown('<div class="hero-badge"><span class="dot"></span> NOW SCREENING · LIVE PIPELINE</div>', unsafe_allow_html=True)
st.markdown('<div class="hero-eyebrow">DATA ENGINEERING · PORTFOLIO PROJECT</div>', unsafe_allow_html=True)
st.markdown('<div class="hero-title">DIRTY DATA<br>CLEANING STUDIO</div>', unsafe_allow_html=True)
st.markdown(
    '<div class="hero-sub">A messy, real-world movie catalogue — duplicate IDs, mixed date '
...
with tab_compare:
    ...
    if st.session_state.cleaned is None:
        ...
        with c3:
            ...
            quality = 100 * (1 - nulls_after / total_cells) if total_cells else 0
            metric_card("Data Quality Score", f"{quality:.0f}/100", "cells non-null", good=True)

        st.write("")
        film_divider()

        # ---- SIDE-BY-SIDE RAW vs CLEANED TABLE ----
        section("Side-by-Side · Raw vs Cleaned", "Same rows, before and after the pipeline touches them.")

        shared_cols = [c for c in df_raw.columns if c in cleaned_df.columns]
        f1, f2 = st.columns([2, 1])
        with f1:
            focus_cols = st.multiselect(
                "Columns to inspect", options=shared_cols,
                default=shared_cols[: min(6, len(shared_cols))],
            )
        with f2:
            n_rows = st.slider("Rows to preview", 5, 50, 12)

        view_cols = focus_cols if focus_cols else shared_cols
        n_preview = min(n_rows, rows_before, rows_after)

        rawL, cleanR = st.columns(2)
        with rawL:
            st.markdown('<div class="subhead">Raw · Before</div>', unsafe_allow_html=True)
            st.dataframe(
                df_raw[view_cols].head(n_preview),
                use_container_width=True, height=380,
            )
        with cleanR:
            st.markdown('<div class="subhead clean">Cleaned · After</div>', unsafe_allow_html=True)
            st.dataframe(
                cleaned_df[view_cols].head(n_preview),
                use_container_width=True, height=380,
            )

        # ---- PER-COLUMN NULL COUNT DIFF ----
        st.write("")
        st.markdown('<div class="subhead">Per-Column Null Count · Raw vs Cleaned</div>', unsafe_allow_html=True)
        null_before_s = df_raw[shared_cols].isnull().sum()
        null_after_s = cleaned_df[shared_cols].isnull().sum()
        diff_table = pd.DataFrame({
            "Column": shared_cols,
            "Nulls (Raw)": [int(null_before_s[c]) for c in shared_cols],
            "Nulls (Cleaned)": [int(null_after_s[c]) for c in shared_cols],
        })
        diff_table["Nulls Fixed"] = diff_table["Nulls (Raw)"] - diff_table["Nulls (Cleaned)"]
        st.dataframe(diff_table, use_container_width=True, height=260, hide_index=True)
...
film_divider()
st.caption("Dirty Data Cleaning Studio — built with pandas, NumPy, Streamlit & Plotly.")